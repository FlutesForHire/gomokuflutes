...
//#region Game States
if (fs.existsSync('source/shared/gamestates.jsonc'))
{
	builder.watchCommand("Game States: 'gamestates.jsonc' => 'states.inc.php', '.action.php', [gamestates.d.ts]", 'source/shared/gamestates.jsonc', () => {
		if (!fs.existsSync('source/shared/gamestates.jsonc')) {
			console.error('File not found: source/shared/gamestates.jsonc.');
			return;
		}

		let statesJSON = jsoncUtil.readObject('source/shared/gamestates.jsonc');
		...
		if (fs.existsSync('source/client/tsconfig.json'))
		{
			if (!fs.existsSync("source/client/build/"))
				fs.mkdirSync('source/client/build/', { recursive: true });

			fs.writeFileSync('source/client/build/gamestates.d.ts',
...
$machinestates = ${writer.stringify(statesJSON, true)};`
		...
		fs.writeFileSync('gomokuflutes.action.php',
`<?php
${writer.fileSignature}
...
//#region JSONC->JSON
{
	...
	if (fs.existsSync('source/shared/stats.jsonc'))
		jsoncToJson("Game Statistics: stats.jsonc => stats.json", 'source/shared/stats.jsonc', 'stats.json');
	if (fs.existsSync('source/shared/gameoptions.jsonc'))
		jsoncToJson("Game Options: gameoptions.jsonc => gameoptions.json", 'source/shared/gameoptions.jsonc', 'gameoptions.json');
	if (fs.existsSync('source/shared/gamepreferences.jsonc'))
		jsoncToJson("Game Preferences: gamepreferences.jsonc => gamepreferences.json", 'source/shared/gamepreferences.jsonc', 'gamepreferences.json');
}
//#endregion

//#region Game Infos
if (fs.existsSync('source/shared/gameinfos.jsonc'))
{
	builder.watchCommand("Game Infos: gameinfos.jsonc => gameinfos.inc.php", 'source/shared/gameinfos.jsonc', () => {
		const gameinfos = jsoncUtil.readObject('source/shared/gameinfos.jsonc');
...
if (fs.existsSync('source/client/tsconfig.json')) {
	...
	const tsCommand = 'npx tsc -p source/client --outFile gomokuflutes.js' + (builder.watch ? ' -w' : '');
	builder.execCommand(`Typescript compiler: client/define.ts => gomokuflutes.js`, tsCommand);

	// const cookbookModuleRegex = /"@cookbook\/([^"]+)"/g;
	// const searchString = 'define("gomokuflutes", [';
	// const importFixer = () => {
	// 	if (!fs.existsSync('gomokuflutes.js'))
	// 		return;

	// 	const data = fs.readFileSync('gomokuflutes.js', 'utf8');
	// 	const start = data.indexOf(searchString);
	// 	if (start !== -1) {
	// 		const end = start + searchString.length;
	// 		const result = data.substring(0, start) + 'define([' + data.substring(end);
	// 		fs.writeFileSync('gomokuflutes.js', result, 'utf8');
	...
	// 		if (imports.length === 0) {
	// 			if (fs.existsSync('source/client/build/imports.d.ts')) {
	// 				fs.unlinkSync('source/client/build/imports.d.ts');
	// 				if (!builder.watch) {
	...
	// 		const importsContent = writer.fileSignature + imports.map(importName => `\n/// <reference path="../../../node_modules/bga-ts-template/typescript/cookbook/${importName}.ts" />`).join('');

	// 		if (!fs.existsSync('source/client/build/imports.d.ts') || fs.readFileSync('source/client/build/imports.d.ts', 'utf8') !== importsContent) {
	// 			if (!fs.existsSync('source/client/build'))
	// 				fs.mkdirSync('source/client/build', { recursive: true });

	// 			fs.writeFileSync('source/client/build/imports.d.ts', importsContent, 'utf8');
	...
	// builder.watchCommand("Import Generator: source/client/build/imports.d.ts", 'gomokuflutes.js', importFixer);
}

if (fs.existsSync('source/client/gomokuflutes.scss')) {
	builder.execCommand(`SCSS compiler: client/gomokuflutes.scss => gomokuflutes.css`, `npx sass --no-source-map source/client/gomokuflutes.scss gomokuflutes.css` + (builder.watch ? ' --watch' : ''));
}
