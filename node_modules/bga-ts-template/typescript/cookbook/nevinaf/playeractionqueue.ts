...
class PlayerActionQueue
{
	 ...
	 * 
	 * If defined and not 'none', the title will always be unlocked as soon as there are no actions in the {@link queue} (waiting or inprogress).
	...
	asyncPostActions()
	{
		...
		for (let i = 0; i < this.queue.length; i++)
		{
			...
			// else state is 'queued'. Items are removed when they are complete or failed.
				...
				this.game.ajaxcall(
					`/${this.game.game_name}/${this.game.game_name}/${item.action}.html`,
		...
		else if (this.queue.every(i => i.state != 'inProgress')) {
